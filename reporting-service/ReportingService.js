    ...
    self.settingsService.readSetting(SETTINGS.enableAcdc, (err, enabled) => {
      ...
      self.settingsService.readSetting(SETTINGS.networkName, (err, networkName) => {
        ...
        this._bindUserDetails(evt, (err, boundEvt) => {
          ...
          let request = {
            url: boundEvt.destination || API_HUB_ENDPOINT,
            body: JSON.stringify(boundEvt),
            options: {method: "post", headers: HEADERS.headers}
          };
          self.http.doPost("/forwardRequestForAuthenticatedClient#x-blockchain-domain-request", JSON.stringify(request), HEADERS, (err, result) => {
            if (err)
              return callback(err);
            try {
